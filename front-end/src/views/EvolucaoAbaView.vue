...
export default {
    ...
    setup() {
        ...
        const agendarconsulta = async () => {
            const cpf = '138.845.747-25';
            try {
                const response = await Axios.get(`http://localhost:3000/consultasAba/paciente/${cpf}`);
        ...
        const processData = () => {
            const labels = consultas.value.map(consulta => new Date(consulta.hora_inicio).toLocaleDateString());
